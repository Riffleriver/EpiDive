...
with tempfile.TemporaryDirectory() as temporary:
    ...
    signature = pipeline._signature(args)
    marker = root / "_SUCCESS.json"
    pipeline._write_json_atomic(
        str(marker), {"analysis_signature": signature, "status": "complete"}
    )
    assert pipeline._matching_marker(str(marker), signature)
    assert not pipeline._matching_marker(str(marker), "different")
    ...
    full_signature = pipeline._signature(args, full_only=True)
