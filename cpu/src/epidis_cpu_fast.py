...
def _score_joint_block_inplace(
    joint11,
    source_dp,
    source_dq,
    target_dp,
    ...
    """Apply the legacy formula in place and retain threshold passes only.
    ...
    The input joint-count matrix is no longer needed after scoring, so it is
    reused as the score matrix.  A conservative squared-threshold guard avoids
    square roots for clear failures; candidates near the boundary still use
    the original sqrt-and-compare operation to preserve legacy selection.
    """
    block_rows, target_count = joint11.shape
    threshold_sq = threshold * threshold
    threshold_guard = max(1e-12, abs(threshold_sq) * 1e-6)
    for local_i in prange(block_rows):
        lower_stop = min(local_i, target_count)
        for target_local in range(lower_stop):
            joint11[local_i, target_local] = -1.0

        dp = source_dp[local_i]
        dq = source_dq[local_i]
        if dp <= 0.0 or dq <= 0.0:
            for target_local in range(lower_stop, target_count):
                joint11[local_i, target_local] = -1.0
            continue
        size_p = dp / (dp + dq)
        size_q = 1.0 - size_p
        for target_local in range(local_i, target_count):
            ...
            if js < threshold_sq - threshold_guard:
                joint11[local_i, target_local] = -1.0
                continue
            score = np.sqrt(js) if js > 0.0 else 0.0
            joint11[local_i, target_local] = (
                score if score >= threshold else -1.0
            )
    return joint11
    ...
    try:
        with limiter:
            for start in iterator:
                ...
                scores = _score_joint_block_inplace(
                    joint11,
                ...
                local_i, target_local = np.nonzero(scores >= 0.0)
                if local_i.size == 0:
                    continue
